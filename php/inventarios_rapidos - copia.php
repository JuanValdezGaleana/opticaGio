...
switch($op){
    ...
    case 1:
            ...
            //$qlinv=$con->prepare('CALL LISTAINVENTARIOSRAPIDOS();');
            $rlinv=$qlinv->execute();
            ...
            if ($rlinv) {
                while($dlinv=$qlinv->fetch()){
                    $datos[]=array('id_inventario'=>$dlinv['ID_INVENTARIO'],
                                   'nombre_inv'=>$dlinv['NOMBRE_INV'],
                                   'fecha_inv'=>$dlinv['FECHA_INV'],
                                   'comentarios_inv'=>$dlinv['COMENTARIOS_INV']);
                }
